    ...
    <div class="container">
        <div class="row ">
            <div class="col-6 d-flex text-left">
                <h1 class=" text-left">فئات العقارات</h1>
            </div>
            <div class="col-6 d-flex justify-content-end">
                @ability('superAdmin', 'manage_buildings,create_buildings')
                <a href="{{ route('admin.buildings.create') }}" class="btn btn-primary font-weight-bolder">
        ...
        @include('backend.buildings.filter')
                ...
                <table class="table table-bordered table-hover table-striped table-light yajra-datatable">
                    <thead class="table-dark ">
                        <tr class="text-light">
                            ...
                            <th class="text-light">اسم الفئة</th>
                            <th class="text-light">عدد العقارات</th>
                            <th class="text-light">الوصف</th>
                            <th class="text-light">الحالة</th>
                            <th class="text-light">العمليات</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($buildings as $k => $category)
                            <tr data-entry-id="{{ $category->id }}">
                                ...
                                <td class="text-center">
                                    {{ $category->products_count }}<br>
                                    <small><a href="{{ route('admin.buildings.show', $category) }}" target="_blank">عرض الكل</a> </small>
                                </td>
                                ...
                                <td class="text-center">
                                    <div style="display: flex" class="text-center justify-content-between">
                                        @ability('superAdmin', 'manage_buildings,update_buildings')
                                            <a href="{{ route('admin.buildings.edit', $category->id) }}"
                                                class="edit btn btn-success btn-sm"><i class="fas fa-edit"></i>
                                            </a>
                                        @endability

                                        @ability('superAdmin', 'manage_buildings,delete_buildings')
                                    ...
                                    <form action="{{ route('admin.buildings.destroy', $category->id) }}" method="post" id="record_delete_{{ $category->id }}" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
            ...
            var table = $('.yajra-datatable').DataTable({
                ...
                buttons: [{
                    ...
                    },
                    {
                        ...
                        text: 'Delete All',
                        url: "{{ route('admin.buildings.buildingsDestroyAll') }}",
        ...
        $(function () {
            $('.status-class').change(function() {
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    url: '{{ route('admin.buildings.buildingCategoriesUpdateStatus') }}',
                    data: {
                        'status': status,
                        'cat_id': cat_id
                    },
