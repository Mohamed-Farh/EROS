                        ...
                        @foreach ($jobs as $k => $job)
                            <tr data-entry-id="{{ $job->id }}">
                                ...
                                <td class="text-center">
                                    ...
                                    <p class="text-gray-400">{{ $job->user->email }}<br>{{ $job->user->mobile }}</p>
                                </td>
                                <td class="text-center">{{ $job->name }}</td>
                                <td>{{ $job->gender == 1 ? 'ذكر' : 'انثي'}}</td>
                                <td class="text-center">{{ $job->jobCategory->name }}</td>
                                <td class="text-center">{{ $job->speciality }}</td>
                                <td class="text-center">{{ $job->description }}</td>
                                <td class="text-center">{{ $job->exp_years }}</td>
                                ...
                                <td class="text-center">
                                    ...
                                    <form action="{{ route('admin.jobs.destroy', $job->id) }}" method="post" id="record_delete_{{ $job->id }}" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
        ...
        $(function () {
            $('.status-class').change(function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{ route('admin.jobs.updateStatus') }}',
                    data: {
                        'status': status,
                        'cat_id': cat_id
                    },
