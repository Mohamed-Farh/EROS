    ...
    <div class="container">
        <div class="card shadow mb-4">
            <div class="card-header py-3 d-flex">
                <div class="col-6">
                    <h6 class="m-0 font-weight-bold text-primary">تعدبل وظيفة </h6>
                </div>
                ...
                <form action="{{ route('admin.jobs.update', $job->id) }}" method="post">
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <input type="text" name="customer_name"
                                    value="{{ $buildingProduct->user->full_name }}" class="form-control" readonly>
                    ...
                    <div class="row mt-3">
                                ...
                                <select name="job_category_id" class="form-control" id="job_category_id" required>
                                    ...
                                    @forelse ($jobCategories as $jobCategory)
                                        <option value="{{ $jobCategory->id }}"
                                            {{ old('job_category_id', $job->job_category_id) == $jobCategory->id ? 'selected' : null }}>
                        ...
                        <div class="col-6">
                            ...
                            <input type="text" name="phone" value="{{ old('phone', $job->phone) }}" class="form-control" required>
                            @error('phone')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="row mt-4">
                        <div class="col-6">
                            ...
                            <input type="text" name="speciality"
                                value="{{ old('speciality', $job->speciality) }}" class="form-control"
                                required>
                            @error('speciality')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="col-3">
                            ...
                            <input type="number" name="exp_years"
                                value="{{ old('exp_years', $job->exp_years) }}" class="form-control" min="1"
                                max="70" required>
                            @error('exp_years')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="col-3">
                            ...
                            <select name="status" class="form-control">
                                <option value="1" {{ old('status', $job->status) == 1 ? 'selected' : null }}>
                                    نشط</option>
                                <option value="0" {{ old('status', $job->status) == 0 ? 'selected' : null }}>
                                    غير نشط</option>
                            </select>
                            @error('status')
                                <span class="text-danger">{{ $message }}</span>
