    ...
    <div class="container">
        <div class="row mb-10">
            <div class="col-6 d-flex text-left">
                <h1 class=" text-left">اقسام الوظائف</h1>
            </div>
            <div class="col-6 d-flex justify-content-end">
                @ability('superAdmin', 'manage_jobs,create_jobs')
                <a href="{{ route('admin.jobCategories.create') }}" class="btn btn-primary font-weight-bolder">
        ...
        @include('backend.jobCategories.filter')
                ...
                <table class="table table-bordered table-hover table-striped table-light yajra-datatable">
                    <thead class="table-dark ">
                        <tr class="text-light">
                            ...
                            <th class="text-light">عدد الوظائف</th>
                            <th class="text-light">الحالة</th>
                            <th class="text-light">العمليات</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($jobCategories as $k => $job)
                            <tr data-entry-id="{{ $job->id }}">
                                <td></td>
                                <td class="text-center">{{ $job->name }}</td>
                                <td class="text-center">{{ $job->jobs->count() }}</td>
                                <td class="text-center">
                                    <span class="switch switch-icon">
                                        <label>
                                            <input data-id="{{ $job->id }}" class="status-class" type="checkbox"
                                ...
                                <td class="text-center">
                                    <div style="display: flex" class="text-center justify-content-between">
                                        @ability('superAdmin', 'manage_jobs,update_jobs')
                                            <a href="{{ route('admin.jobCategories.edit', $job->id) }}"
                                                class="edit btn btn-success btn-sm"><i class="fas fa-edit"></i>
                                            </a>
                                        @endability

                                        @ability('superAdmin', 'manage_jobs,delete_jobs')
                                            <a href="javascript:void(0)"
                                                onclick="
                                                    if (confirm('Are You Sure You Want To Delete This Record ?') )
                                                        { document.getElementById('record_delete_{{ $job->id }}').submit(); }
                                                    else
                                                        { return false; }"
                                    ...
                                    <form action="{{ route('admin.jobCategories.destroy', $job->id) }}" method="post" id="record_delete_{{ $job->id }}" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
            ...
            var table = $('.yajra-datatable').DataTable({
                ...
                buttons: [{
                    ...
                    },
                    {
                        ...
                        text: 'Delete All',
                        url: "{{ route('admin.jobCategories.massDestroy') }}",
        ...
        $(function () {
            $('.status-class').change(function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{ route('admin.jobCategories.updateStatus') }}',
                    data: {
                        'status': status,
                        'cat_id': cat_id
                    },
