    ...
    <div class="container">
        <div class="row ">
            <div class="col-6 d-flex text-left">
                <h1 class=" text-left">انواع السيارات</h1>
            </div>
            <div class="col-6 d-flex justify-content-end">
                @ability('superAdmin', 'manage_cars,create_cars')
                <a href="{{ route('admin.carTypes.create') }}" class="btn btn-primary font-weight-bolder">
        ...
        @include('backend.carTypes.filter')
        ...
        <div class="row">
            <div class="col-12">
                <table class="table table-bordered table-hover table-striped table-light yajra-datatable">
                    <thead class="table-dark ">
                        <tr class="text-light">
                            ...
                            <th class="text-light">عدد السيارات</th>
                    ...
                    <tbody>
                        @foreach ($carTypes as $k => $carType)
                            <tr data-entry-id="{{ $carType->id }}">
                                <td></td>
                                <td class="text-center">
                                    <img class="rounded" width="90" height="60"
                                        src="{{ asset($carType->cover) }}">
                                </td>
                                <td class="text-center">{{ $carType->name }}</td>
                                <td class="text-center">
                                    {{ $carType->products_count }}<br>
                                    <small><a target="_blank"></a> </small>
                                </td>
                                <td class="text-center">
                                    <span class="switch switch-icon">
                                        <label>
                                            <input data-id="{{ $carType->id }}" class="status-class" type="checkbox"
                                                data-onstyle="success" data-offstyle="danger" data-toggle="toggle" data-on="On"
                                                data-width="40" data-height="30" data-off="Off"
                                                {{ $carType->status == 1 ? 'checked' : '' }}>
                                            <span></span>
                                        </label>
                                    </span>
                                </td>
                                <td class="text-center">
                                    <div style="display: flex" class="text-center justify-content-between">
                                        @ability('superAdmin', 'manage_cars,update_cars')
                                            <a href="{{ route('admin.carTypes.edit', $carType->id) }}"
                                                class="edit btn btn-success btn-sm"><i class="fas fa-edit"></i>
                                            </a>
                                        @endability

                                        @ability('superAdmin', 'manage_cars,delete_cars')
                                            <a href="javascript:void(0)"
                                                onclick="
                                                    if (confirm('Are You Sure You Want To Delete This Record ?') )
                                                        { document.getElementById('record_delete_{{ $carType->id }}').submit(); }
                                                    else
                                                        { return false; }"
                                    ...
                                    <form action="{{ route('admin.carTypes.destroy', $carType->id) }}" method="post" id="record_delete_{{ $carType->id }}" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
                ...
                <div class="d-flex justify-content-center">
                    {!! $carTypes->appends(request()->input())->links() !!}
                </div>
            </div>
        </div>
    </div>
            ...
            var table = $('.yajra-datatable').DataTable({
                ...
                buttons: [{
                    ...
                    },
                    {
                        ...
                        text: 'Delete All',
                        url: "{{ route('admin.carTypes.massDestroy') }}",
        ...
        $(function () {
            $('.status-class').change(function() {
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    url: '{{ route('admin.carTypes.updateStatus') }}',
                    data: {
                        'status': status,
                        'cat_id': cat_id
                    },
