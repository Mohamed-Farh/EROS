    ...
    <div class="container">
        <div class="row ">
            <div class="col-6 d-flex text-left">
                <h1 class=" text-left">الصفحة الرئيسية (عن المركز)</h1>
            </div>
            <div class="col-6 d-flex justify-content-end">
                @ability('superAdmin', 'manage_home,show_home_abouts')
                <a href="{{ route('admin.home_abouts.create') }}" class="btn btn-primary font-weight-bolder">
        ...
        <div class="row">
            <div class="col-12">
                <table class="table table-bordered table-hover table-striped table-light yajra-datatable">
                    ...
                    <tbody>
                        @foreach ($home_abouts as $k => $home_about)
                            <tr data-entry-id="{{ $home_about->id }}">
                                ...
                                <td class="text-center">
                                    @if ($home_about->video != '')
                                        <a href="{{ $home_about->video }}" target="_blank">عرض الفيديو</a>
                                    @else
                                        ---
                                    @endif
                                </td>
                                @ability('superAdmin', 'manage_home,show_home_abouts')
                                    <td class="text-center">
                                        <span class="switch switch-icon">
                                            <label>
                                                <input data-id="{{ $home_about->id }}" class="status-class" type="checkbox"
                                                    data-onstyle="success" data-offstyle="danger" data-toggle="toggle"
                                                    data-on="On" data-width="40" data-height="30" data-off="Off"
                                                    {{ $home_about->status == 1 ? 'checked' : '' }}>
                                                <span></span>
                                            </label>
                                        </span>
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <div style="display: flex" class="text-center justify-content-between">
                                            ...
                                            <a href="{{ route('admin.home_abouts.edit', $home_about->id) }}"
                                                class="edit btn btn-success btn-sm"><i class="fas fa-edit"></i>
                                            </a>

                                            <a href="javascript:void(0)" onclick="
                                                                if (confirm('Are You Sure You Want To Delete This Record ?') )
                                                                    { document.getElementById('record_delete_{{ $home_about->id }}').submit(); }
                                                                else
                                                                    { return false; }" class="btn btn-danger"><i
                                                    class="fa fa-trash"></i>
                                            </a>
                                        </div>
                                        <form action="{{ route('admin.home_abouts.destroy', $home_about->id) }}" method="post"
                                            id="record_delete_{{ $home_about->id }}" class="d-none">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
                ...
                <div class="d-flex justify-content-center">
                    {!! $home_abouts->appends(request()->input())->links() !!}
                </div>
            </div>
        </div>
    </div>
            ...
            var table = $('.yajra-datatable').DataTable({
                ...
                buttons: [{
                    ...
                    },
                    {
                        ...
                        text: 'Delete All',
                        url: "{{ route('admin.home_abouts.massDestroy') }}",
        ...
        $(function () {
            $('.status-class').change(function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{ route('admin.home_abouts.changeStatus') }}',
                    data: {
                        'status': status,
                        'cat_id': cat_id
                    },
