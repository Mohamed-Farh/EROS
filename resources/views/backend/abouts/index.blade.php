    ...
    <div class="container">
        <div class="row ">
            ...
            <div class="col-6 d-flex justify-content-end">
                @ability('superAdmin', 'manage_home,show_abouts')
                <a href="{{ route('admin.abouts.create') }}" class="btn btn-primary font-weight-bolder">
                        ...
                        @foreach ($abouts as $k => $about)
                            <tr data-entry-id="{{ $about->id }}">
                                ...
                                <td class="text-center">
                                    <img class="rounded" width="90" height="60" src="{{ asset($about->image) }}" alt="">
                                </td>
                                <td class="text-center">{{ $about->title }}</td>
                                <td class="text-center">{{ $about->text }}</td>
                                <td class="text-center">
                                    @if ($about->video != '')
                                        <a href="{{ $about->video }}" target="_blank">عرض الفيديو</a>
                                    @else
                                        ---
                                    @endif
                                </td>
                                @ability('superAdmin', 'manage_home,show_abouts')
                                    <td class="text-center">
                                        <span class="switch switch-icon">
                                            <label>
                                                <input data-id="{{ $about->id }}" class="status-class" type="checkbox"
                                    ...
                                    <td class="text-center">
                                        <div style="display: flex" class="text-center justify-content-between">
                                            ...
                                            <a href="javascript:void(0)" onclick="
                                                                if (confirm('Are You Sure You Want To Delete This Record ?') )
                                                                    { document.getElementById('record_delete_{{ $about->id }}').submit(); }
                                                                else
                                        ...
                                        <form action="{{ route('admin.abouts.destroy', $about->id) }}" method="post"
                                            id="record_delete_{{ $about->id }}" class="d-none">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
        ...
        $(function () {
            $('.status-class').change(function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: '{{ route('admin.abouts.changeStatus') }}',
                    data: {
                        'status': status,
                        'cat_id': cat_id
                    },
